...
export default function HistoryTable({ items, compact = false, onDone }) {
  ...
  function openRequeue(record) {
    openModal(t('requeue'), <HistoryRequeueForm record={record} onDone={onDone} />);
  }
              ...
              {canManage && (
                <td>
                  <div className="row-actions">
                    {me.is_admin && <button className="btn secondary small" onClick={() => openRequeue(i)}>{t('requeue')}</button>}
